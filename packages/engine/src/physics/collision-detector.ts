...
import type { CollisionResult, CollisionGeometry } from './types';
...
export class CollisionDetector {
  ...
  public setGeometry(geometry: CollisionGeometry): void {
    ...
    Logger.info(`[COLLISION] Loaded geometry: ${geometry.lineDefs.length} lines, ${geometry.sectors.length} sectors`);
  }
  ...
  public testCircleLineCollision(
    ...
    if (!this.geometry) {
      return { collided: false, correction: Vector2.Zero(), normal: Vector2.Zero(), distance: 0 };
    }
  ...
  private testCircleLineSegment(
    ...
    const collisionNormal = distanceToLine > 0.001 
  ...
  private isPointInSector(point: Vector2, sector: DoomSector): boolean {
    ...
    for (let i = 0, j = vertices.length - 1; i < vertices.length; j = i++) {
      ...
      if (((viPos.y > point.y) !== (vjPos.y > point.y)) &&
